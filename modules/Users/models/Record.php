...
class Users_Record_Model extends Vtiger_Record_Model {
	...
	public function getAccessibleUsers($private="",$module = false) {
	    ...
	    /**********************************  27 Jul 2018 Change  for accessible users END  *************************/
	    ...
	    if(!$this->isAdminUser() && $accessible_lists){
	        ...
	        if($accessible_lists){
	            ...
	            $accessible_users = Vtiger_Functions::computeCRMRecordLabels("Users", $accessible_lists);
	            if(!empty($accessible_users))
	                $accessibleUser = $accessible_users;
	        }
	        
	        if(empty($accessibleUser)){
	            
	            $accessibleGroupUsers = array();
	            
	            $groups = new GetUserGroups();
	            
	            $groups->getAllUserGroups($this->getId());
	            
	            $groups = $groups->user_groups;
	            
	            // 1. all users that belong to group of which he is a part
	            require_once('include/utils/GetGroupUsers.php');
	            if(!empty($groups)){
	                foreach($groups as $group){
	                    $groupUsers = new GetGroupUsers();
	                    $groupUsers->getAllUsersInGroup($group);
	                    $accessibleGroupUsers = array_merge($accessibleGroupUsers, $groupUsers->group_users);
	                }
	            }
	            
	            // 2. His own user
	            if(!in_array($this->getId(),$accessibleGroupUsers))
	                $accessibleGroupUsers = array_merge($accessibleGroupUsers,array($this->getId()));
	                
	                $accessibleUser = Vtiger_Functions::computeCRMRecordLabels("Users", $accessibleGroupUsers);
	                
	                // 3. His all users belong to role under him
	                $roleBasedAccessibleUser = $this->getRoleBasedSubordinateUsers();
	                
	                if(!empty($roleBasedAccessibleUser))
	                    $accessibleUser = $accessibleUser+$roleBasedAccessibleUser;
	        }
	    } else {
    	...
    	if(!isset($accessibleUser[$this->getId()]))
    	    $accessibleUser[$this->getId()] = getUserFullName($this->getId());
	...
	public function getAccessibleGroups($private="",$module = false) {
        ...
        if(empty($accessibleGroups)) return array();
        
        if($this->isAdminUser()) return $accessibleGroups;
        ...
        if(!empty($groups->user_groups)){
            
            foreach($groups->user_groups as $groupid){
                $usersGroups[$groupid] = $accessibleGroups[$groupid];
            }
        }
        ...
        if($accessible_lists){
            ...
            foreach($allGroups as $group_id => $group_name){
                if(!in_array($group_id, $accessible_lists))
                    unset($accessibleGroups[$group_id]);
            }
            
            if(empty($accessibleGroups))
                $accessibleGroups = $usersGroups;
        } else
            $accessibleGroups = $usersGroups;
    ...
    public function getRelatedGroupsInformation(){
        ...
        if(!empty($groups->user_groups)){
            ...
            $query = "SELECT * FROM vtiger_groups WHERE groupid IN (".generateQuestionMarks($groups->user_groups).")";

            $result = $db->pquery($query, array($groups->user_groups));

            while($row = $db->fetchByAssoc($result)){

                $group_info[] = array(
                    "name"=>decode_html($row["groupname"]),
                    "id"=>$row['groupid'],
            ...
            /*foreach($groups->user_groups as $groupid){
             ...
             $result = $db->pquery($query, array($groupid));

             $group_info[] = array(
                 "name"=>decode_html($db->query_result($result, 0, "groupname")),
                 "id"=>$groupid,
	...
	function getCurrentUserSubscription($type){
	    ...
	    if($type == 'Calendar')
	        $query = "select * from vtiger_user_subscription where userid = ?";
        else if($type == 'Contacts')
            $query = "select * from vtiger_user_contacts_subscription where userid = ?";
