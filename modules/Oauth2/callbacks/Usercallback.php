...
class Oauth2_Usercallback_Callbacks {
    ...
    public static function handleRequest($config, $req) {
        ...
        if (!isset($req['code'])) {
            ...
            $payload  = $parsed_url['host']."/||".$current_user->id."||OutgoingServer||".$req['authfor']."||".$req['authservice'];
            
			if(isset($req['server_username']) && $req['server_username']) {
                $payload .= "||".$req['server_username'];
            }
			...
			$_SESSION['oauth2state'] = $state; //$provider->getState();
            ...
            try {
				...
				if($oauth2svc == 'Office365') {
                    ...
                    $userinfo['email'] = $req['server_username'];
                    static::updateTokensFor($config, $oauth2for, $oauth2svc, $userinfo, $tokens, $accessTokenExpiresOn);
				/*} else if($oauth2svc == 'Outlook') {
					//$resourceOwner = $provider->getResourceOwner($accessToken);
					//$userinfo = $resourceOwner ? $resourceOwner->toArray() : null;
					$userinfo['email'] = 'devitechno_manish@outlook.com'; //$userinfo['mail'];
					$userinfo['email_verified'] = 1; //$userinfo['mail'];*/
				} else {
					$resourceOwner = $provider->getResourceOwner($accessToken);
					$userinfo = $resourceOwner ? $resourceOwner->toArray() : null;
					
					if($oauth2svc == 'Outlook'){
    ...
    protected static function updateTokensFor($config, $oauth2for, $oauth2svc, $userinfo, $tokens, $expireson) {
        ...
        if ($oauth2for == "OutgoingServer") {
            ...
            if ($db->num_rows($checkRs)) {
                $db->pquery("update vtiger_systems set server = ?, server_port = ?, server_username = ?, server_password = ?, smtp_auth = ?, smtp_auth_type = ?, smtp_auth_expireson = ?, from_email_field = ? where server_type = ?",
                    array(
                        $server,
                        $port,
                        $userinfo["email"],
                ...
                $db->pquery("insert into vtiger_systems (id, server, server_port, server_username, server_password, smtp_auth, smtp_auth_type, smtp_auth_expireson, from_email_field, server_type) values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                    array(
			...
			if(strcasecmp($oauth2svc, "Google") === 0){
				...
				$server = strcasecmp($oauth2svc, "Outlook") === 0? "outlook.office365.com" : "";
			}
